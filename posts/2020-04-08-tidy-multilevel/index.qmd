...
>
> - `\(x)` anonymous function shorthand now recommended inside `map()`, according to purrr >= 1.0.0 and R >= 4.0 style
> - `geom_pointrange()` is now used to plot points and error bars together, instead of `geom_errorbar() + geom_point()`. I like `geom_pointrange()` better now for conciseness!
...
![The fruit that launched a thousand guacamoles?](https://upload.wikimedia.org/wikipedia/commons/thumb/2/25/Avocado_open_side_by_side.jpg/640px-Avocado_open_side_by_side.jpg){fig-align="center"}
...
The code you put in the second argument is preceded with `\(x)`, [which (as of R >= 4.0) is a shortcut to define an anonymous function.](https://www.jumpingrivers.com/blog/new-features-r410-pipe-anonymous-functions/) An [anonymous function](https://adv-r.hadley.nz/functions.html#first-class-functions) is kind of like a "naked" function, where its underlying is showing without being assigned to a function name. Because the function is defined with no name, it can't be reused without copying and pasting the code. But the upside is that you can define a quick little function to do one small thing without needing to store it in a function name!
...
>**Instructor's note:** [Since December 2022, the purrr maintainers now recommend using `\(x)` anonymous function shorthand over formula syntax inside of `map()`.](https://www.tidyverse.org/blog/2022/12/purrr-1-0-0/#documentation) Tilde `~` formula-style syntax still works, as does calling a bare function names as the object, but I recommend teaching anonymous function shorthand _only_ for non-advanced learners. Avoiding confusion is more important than a comprehensive explanation at this level.
